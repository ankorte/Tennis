...
export default function Layout() {
  ...
  // Mehr-Menü schließen bei Klick außerhalb
  useEffect(() => {
  ...
  // Session-Timeout-Warnung (JWT läuft nach 24h ab)
  ...
  useEffect(() => {
    ...
    try {
      ...
      const warnAt = expiresAt - 10 * 60 * 1000 // 10 Min vorher warnen
      const msUntilWarn = warnAt - Date.now()
      const msUntilExpiry = expiresAt - Date.now()
      if (msUntilWarn <= 0 && msUntilExpiry > 0) {
        setShowExpiry(true)
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-50 shadow-lg"
        ...
        {/* Schmaler roter Akzentstreifen oben */}
        ...
        <div className="px-3 py-2 flex items-center gap-3">
          ...
          <div className="text-right flex-shrink-0">
            ...
            <button onClick={handleLogout}
              className="text-xs underline block" style={{ color: '#FF9DB5' }}>
              Abmelden
            </button>
          </div>
        </div>
      ...
      {totalItems > 0 && (
        <button
          ...
          className="fixed left-0 right-0 z-40 flex items-center justify-between px-4 py-3 shadow-lg"
          style={{
            bottom: '64px',
            background: 'linear-gradient(90deg, #E8002D 0%, #b5001f 100%)',
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <div className="text-white font-black text-sm leading-tight">
                {totalItems} ungebuchte{totalItems === 1 ? 's Getränk' : ' Getränke'}
              </div>
              <div className="text-white/80 text-xs">Tippen zum Abschließen der Buchung</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-white font-black text-lg">{totalPrice.toFixed(2)} €</div>
            <div className="text-white/80 text-xs">Jetzt buchen →</div>
          </div>
        </button>
      )}
      ...
      {showMore && (
        <div className="fixed inset-0 bg-black/30 z-50" onClick={() => setShowMore(false)}>
          <div ref={moreRef}
            ...
            className="absolute bottom-[68px] left-2 right-2 bg-white rounded-2xl shadow-2xl p-4 max-h-[60vh] overflow-y-auto"
            style={{ borderBottom: '3px solid #E8002D' }}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {isThekenwart && (
                <button onClick={() => { navigate('/inventory'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">📦</span>
                  <span className="text-xs font-medium text-gray-700">Lager</span>
                </button>
              )}
              {isKassenwart && (
                <button onClick={() => { navigate('/billing'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">💶</span>
                  <span className="text-xs font-medium text-gray-700">Abrechnung</span>
                </button>
              )}
              {isKassenwart && (
                <button onClick={() => { navigate('/members'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">👤</span>
                  <span className="text-xs font-medium text-gray-700">Mitglieder</span>
                </button>
              )}
              {isThekenwart && (
                <button onClick={() => { navigate('/drinks'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">🍺</span>
                  <span className="text-xs font-medium text-gray-700">Getränke</span>
                </button>
              )}
              {isKassenwart && (
                <button onClick={() => { navigate('/admin/bookings'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">📋</span>
                  <span className="text-xs font-medium text-gray-700">Buchungen</span>
                </button>
              )}
              {isThekenwart && (
                <button onClick={() => { navigate('/admin/carts'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">🛒</span>
                  <span className="text-xs font-medium text-gray-700">Warenkörbe</span>
                </button>
              )}
              {isThekenwart && (
                <button onClick={() => { navigate('/admin/stats'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">📊</span>
                  <span className="text-xs font-medium text-gray-700">Statistiken</span>
                </button>
              )}
              {isKassenwart && (
                <button onClick={() => { navigate('/import'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">📥</span>
              ...
              {isKassenwart && (
                <button onClick={() => { navigate('/sepa'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">🏦</span>
                  <span className="text-xs font-medium text-gray-700">SEPA</span>
                </button>
              )}
              {isKassenwart && (
                <button onClick={() => { navigate('/email'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">📧</span>
              ...
              {isAdmin && (
                <button onClick={() => { navigate('/database'); setShowMore(false) }}
                  className="flex flex-col items-center gap-1 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-2xl">🗄️</span>
                  <span className="text-xs font-medium text-gray-700">Datenbank</span>
                </button>
              )}
      ...
      {/* Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-1 z-50 shadow-lg safe-area-inset-bottom"
        ...
        <NavLink to="/book" className={({ isActive }) =>
          `flex flex-col items-center px-2 py-1.5 rounded-xl transition-colors ${isActive ? 'text-tennis-green' : 'text-gray-400'}`}>
          <span className="text-2xl">🍺</span>
          <span className="text-xs mt-0.5">Buchen</span>
        </NavLink>
        ...
        {isThekenwart && (
          <button onClick={() => setShowMore(!showMore)}
            className={`flex flex-col items-center px-2 py-1.5 rounded-xl transition-colors ${showMore ? 'text-tennis-green' : 'text-gray-400'}`}>
            <span className="text-2xl">☰</span>
